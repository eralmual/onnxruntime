...
inline int calc_num_threads() {
  ...
  if (GetLogicalProcessorInformation(buffer, &returnLength) == FALSE) {
    num_treads = std::thread::hardware_concurrency();
  } else {
...
class DnnlThreadPool : public DnnlThreadPoolIface {
 public:
  explicit DnnlThreadPool(T* threadpool) : inner_treadpool_(threadpool) {}
...
class DnnlORTThreadPool : public DnnlThreadPool<ORTThreadPool> {
 public:
  ...
  int get_num_threads() const override { return inner_treadpool_->NumThreads(); }
  bool get_in_parallel() const override { return inner_treadpool_->CurrentThreadId() != -1; }
  uint64_t get_flags() const override { return !ASYNCHRONOUS; }

  void parallel_for(int n, const std::function<void(int, int)>& fn) override {
    // TryBatchParallelFor uses a function with just one argument so we fix this here by using lambda fn
    inner_treadpool_->TryBatchParallelFor(
        inner_treadpool_, n, [&](ptrdiff_t i) { fn(static_cast<int>(i), n); }, 0);
  }
};
 ...
 public:
  ...
  explicit DnnlEigenThreadPool(int num_threads = 0) :
            DnnlThreadPool(num_threads <= 0 ?
                          (new EigenThreadPool(calc_num_threads())):
  ...
  void parallel_for(int n, const std::function<void(int, int)>& fn) override {
    ...
    for (int i = 0; i < njobs; i++) {
      inner_treadpool_->Schedule([i, n, njobs, fn]() {
        int start, end;
